...
**Clinic**, klinik yönetim süreçlerini yönetmek için geliştirilmiş bir platformdur. Bu repo, platformun **veritabanı şemasını ve migration dosyalarını** barındırır.

- **Repo:** `clinic-database`
- **Veritabanı:** PostgreSQL
- **Schema:** `clinic`
- **Migration Tool:** Flyway 10

**Kardeş projeler:**
- `clinic-service` — Spring Boot backend
- `clinic-web` — React web uygulaması
- `clinic-mobile` — Flutter mobil uygulama
...
- **Uygulama kullanıcısı** — `clinic_app` DB kullanıcısına schema üzerinde tam yetki verilmiştir (V7)
...
| V7 | `V7__grant_clinic_app_permissions.sql` | `clinic_app` kullanıcısına yetki |
...
└── migrations/
    ...
    └── V7__grant_clinic_app_permissions.sql
```

### 4.2 Flyway Konfigürasyonu

```
flyway.url=jdbc:postgresql://${DB_HOST:-localhost}:${DB_PORT:-5432}/postgres?currentSchema=clinic
flyway.user=${DB_USER:-postgres}
flyway.password=${DB_PASSWORD:-123456789}
flyway.schemas=clinic
...
| DB Yetkileri | ✅ Tamamlandı | `clinic_app` kullanıcısına tam yetki |
